...
media.get('/', authMiddleware, async (c) => {
  try {
    // 1. Cek apakah Binding R2 terhubung
    if (!c.env.MY_BUCKET) {
      return c.json({ success: false, error: "❌ Gawat! Binding 'MY_BUCKET' tidak ditemukan di wrangler.toml" }, 500);
    }
    ...
    const data = files.map(f => ({ 
        ...f, 
        url: `/api/media/file/${f.key}` // Pastikan URL ini sesuai route
    }));

    return c.json({ success: true, data });

  } catch (e: any) {
    // Ini akan menampilkan error asli ke browser
    return c.json({ success: false, error: "Server Error: " + e.message }, 500);
  }
})
